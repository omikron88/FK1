...
V emulátoru odpovídá připojené tiskárně hostitelský výstupní soubor. Bez něj je
PC5 v nule (offline), PA bajt se nepřevezme a `/ACK_A` nevznikne. S připojeným
souborem je PC5 v jedničce, přijatý osmibitový bajt se bez převodu zařadí do
výstupní fronty a virtuální tiskárna vytvoří `/ACK_A`. PC4 zůstává v nule,
protože souborový sink nemá stav PAPER END.
...
Osm datových vodičů klávesnice má inverzní polaritu. Logický kód klávesy se
proto před zachycením na PB bitově invertuje; například `A = 0x41` se z 8255
čte jako `0xBE` a `Ctrl+@ = 0x00` jako `0xFF`. Potvrzuje to test `SQ01` v
`DIAG.MAC`, který po čtení PB provádí instrukci `CPL`.
...
- po držení přibližně 0,5 s začne opakovat stejný kód rychlostí přibližně
  4 znaky za sekundu.

Při současném Ctrl se znaky `@` až `_` převádějí terminálovým pravidlem

```c
code = ascii_character & 0x1F;
```

tedy `Ctrl+@ = 0x00`, `Ctrl+A = 0x01`, …, `Ctrl+Z = 0x1A` až
`Ctrl+_ = 0x1F`.

| Klávesa FK-1 | Hostitelská klávesa | Kód | Se Shift |
|---|---|---:|---:|
| Escape | Esc | `0x1B` | stejný |
| Backspace | Backspace | `0x7F` | stejný |
| Tab | Tab | `0x09` | stejný |
| Enter | Enter | `0x0D` | stejný |
| ROL | Delete | `0x80` | `0x81` |
| COPY | Insert | `0x82` | `0x83` |
| BREAK | End | `0x84` | `0x85` |
| nahoru | šipka nahoru | `0xC1` | stejný |
| dolů | šipka dolů | `0xC2` | stejný |
| vpravo | šipka vpravo | `0xC3` | stejný |
| vlevo | šipka vlevo | `0xC4` | stejný |
| Home | Home | `0x8D` | stejný |
| uživatelská 1 | F1 | `0xD0` | stejný |
| uživatelská 2 | F2 | `0xD1` | stejný |
| uživatelská 3 | F3 | `0xD2` | stejný |
...
- při čtení vznikne I6 Disk timeout pouze tehdy, když je aktivní `RE` i `RDM`
  a obvod E14/8 čeká na datovou značku patřící k nalezenému záhlaví,
- při zápisu se aktivuje zápisový signál mechaniky; vypne se s koncem diskové operace.

Samotné hledání adresní značky (`RE=1`, `RDM=0`) TOUT neaktivuje. Software tak
může porovnávat po sobě jdoucí ID pole a projít i přes dlouhou mezeru u Indexu.
Toto chování odpovídá komentářům testů čítače a diskové logiky v
`doc/DIAG.MAC` a zapojení klopného obvodu E14/8 na listu FLOPY II.
...
Hostitelský raw TCP most zachovává tuto bitovou cestu. Přijatý síťový bajt se
podle aktuálního mode word sestaví na start, datové, paritní a stop bity a
posouvá se do RxD hranami RxC. Vysílaný bajt opustí emulátor až po dokončení
rámce hranami TxC. TCP spojení aktivuje vstupy CTS a DSR; po rozpojení je
deaktivuje a I2 se znovu odvodí ze skutečného stavu 8251.
...
Při `RDM=0` může čtecí rutina opakovaně načítat a porovnávat ID pole bez I6.
Po nalezení správného ID přepne na `RDM=1`; teprve tehdy E14/8 hlídá, zda se
datová značka objeví v časovém okně čítače 1.
...
5. software současně pozoruje hranu Indexu přímo na PC7 řídicí 8255.

Při formátování se nepočítají bajty ani čas. Zápis trvá přesně jednu otáčku
mezi dvěma po sobě jdoucími Indexy. Normální dokončení FOR nevytváří I6; I6
zůstává chybovým výstupem TOUT.
...
struct FMByte {
    uint8_t data;
    uint8_t clocks; // FF, D7, EF nebo C7
};
```
...
Původní diagnostický program `doc/DIAG.MAC` polaritu potvrzuje: při RESTORE
používá PC1 = 0 a při pohybu k vyšším stopám PC1 = 1. Platný krok dokončuje
přechodem PC0 z 1 do 0, tedy koncem aktivního impulzu `/STEP`.
...
List 5 schématu a původní popis zapojení určují spodních šest bitů:

| Bit | Signál | Význam |
|---:|---|---|
| PD0 | AX | fáze A vodorovné osy |
| PD1 | BX | fáze B vodorovné osy |
| PD2 | AY | fáze A svislé osy |
| PD3 | BY | fáze B svislé osy |
| PD4 | T1 | tlačítko 1, aktivní v nule |
| PD5 | T2 | tlačítko 2, aktivní v nule |

AX/BX/AY/BY/T1/T2 nejprve invertují Schmittovy členy 74LS14 E57 a znovu je
invertuje výstupní registr 8283 E53, takže procesor vidí jejich původní úrovně.
T1 a T2 mají pull-up: klidový stav spodních šesti bitů je `0x30` při fázích
obou os `00`.

Kulička s optickými clonkami vytváří na každé ose kvadraturní Grayův kód.
Mechanické uspořádání os není stranově shodné: doprava používá
`00 → 10 → 11 → 01 → 00` (BX vede AX), zatímco dolů používá
`00 → 01 → 11 → 10 → 00` (AY vede BY). Opačné směry pořadí obracejí. Jedna
hrana vzniká každou 1 ms emulovaného času. Současný pohyb obou os se střídá po
hranách, aby zůstal zachován diagonální směr. Fyzické fáze se mění i při PA3=0,
pouze se tehdy nezachytí I3.
...
Formát odpovídá geometrii standardního osmipalcového IBM FM média. Přesné
rozložení níže je potvrzené referenční implementací převodu FK-1.
...
Sektorový `.8sd` obraz obsahuje přesně `77 × 26 × 128 = 256 256` bajtů v
logickém pořadí stopa–sektor. Jedna rozvinutá stopa má 5 208 FM bajtů:

| Část | Obsah |
|---|---|
| pre-index gap | 40× `FF` |
| sync + IAM | 6× `00`, potom data `FC` / hodiny `D7` |
| post-index gap | 26× `FF` |
| ID pole | 6× `00`, `FE/C7`, C, H=0, R, N=0, CRC hi/lo |
| ID gap | 11× `FF` |
| datové pole | 6× `00`, `FB/C7`, 128 dat, CRC hi/lo |
| data gap | 27× `FF` |
| opakování | ID a datové pole 26krát |
| pre-index doplnění | 247× `FF` |

Logické sektory 1–26 se ukládají s fyzickým skewem 6. Mapa vzniká algoritmem
`map[i] = sector; i += 6; if (i >= 26) i -= 25` a začíná pořadím
`1, 22, 18, 14, 10, 6, 2, …`. Při 384 master tickách na FM bajt zabere obsah
stopy 1 999 872 ticků a posledních 128 ticků zůstává do přesné hranice otáčky.
...
Používá se CRC-16 s polynomem `0x1021`, počáteční hodnotou `0xFFFF`, bez
závěrečného XOR. Výpočet zahrnuje Address/Data Mark (`FE` nebo `FB`) a celé
následující pole. Na stopu se zapisuje nejprve horní a potom dolní bajt CRC.

### 14.5 Zápis a export sektorového obrazu

Náběžná hrana `OUT1` při aktivním WRE překopíruje první předem připravený bajt
z PA do zápisového posuvného registru a vytvoří první `/ACK_A`. Další bajty se
přebírají po 384 master tickách. Běžné 128bajtové datové pole používá 137
handshake přenosů: 6× `00`, DAM, 128 dat a 2 CRC. `OUT2` po posledním CRC
vynuluje WRE.

Zápisový detektor po nejméně šesti nulových sync bajtech přidělí hodnotě `FC`
hodinovou masku `D7` a hodnotám `FE`, `FB` a `F8` masku `C7`; ostatní bajty
mají běžnou masku `FF`. Při FOR se stejný serializér spustí prvním Indexem a
druhý Index jej zastaví ještě před zápisem další buňky s tickem 0.

Zpětný převod do sektorového `.8sd` prochází každou low-level stopu, spáruje
platné `FE/C7` ID pole s následujícím `FB/C7` nebo `F8/C7` datovým polem a podle
C/H/R/N uloží 128 datových bajtů do logického pořadí stopa–sektor. Export je
odmítnut, pokud chybí kterýkoli z 77 × 26 sektorů; poškozená nebo nedokončená
stopa proto nepřepíše původní obraz.
...
1. Přesná šířka a aktivní hrana Indexu konkrétní mechaniky.
2. Přesné programované režimy a počáteční hodnoty jednotlivých kanálů 8253 mimo sekvence ověřené boot ROM.
3. Elektrická cesta zrušení I6 při vynulování pomocného registru; chování je ověřené softwarem, ale zbývá potvrdit konkrétní hradlo ve schématu.
4. Potvrzení řídicích slov `0xA6` pro datovou 8255 a `0x88` pro řídicí 8255 podle originálního programu nebo schématu.
5. Zapojení a výchozí úrovně ostatních modemových signálů 8251 (`CTS`, `DSR`, `RTS`, `DTR`).
6. Mechanická časování kroku, ustálení hlavy a případné prodlevy Head Load.
7. Přesné chování při současném RE a WRE; software by tuto kombinaci neměl používat, pokud není ve schématu vzájemně blokována.
